...
size_t lcomp(struct line* text, size_t line1, size_t line2);

size_t rcomp(struct line* text, size_t line1, size_t line2);

void change(struct line* text, size_t line1, size_t line2);

void bubblesort(struct line* text, size_t linenum, size_t(* comp)(struct line* text, size_t line1, size_t line2));
...
void get_text(struct line* text, char* fixed_buffer, char* buffer)
{
    ...
    for (size_t i = 0; buffer[i] != '\0'; i++) {
        if (buffer[i] == '\n') {
            ...
            text[count].lengh = (i + 1) - ptr_shift;
            ptr_shift = i + 1;
            count++;
        }
    }
...
void bubblesort(struct line* text, size_t linenum, size_t(* comp)(struct line* text, size_t line1, size_t line2))
{
    for (size_t i = linenum-1; i > 1; i--) {
        assert(i<linenum);
        for(size_t j = 1; j <= i; j++) {
            if (comp(text, j-1, j) == 2) {
                change(text, j-1, j);
            }
        }
    }
...
size_t rcomp(struct line* text, size_t line1, size_t line2)
{
    ...
    for (size_t i = 0; i < (lengh2 - 1) && i < (lengh2 - 1); i++) {
        // Берем символы с конца строк
        int c1 = tolower(str1[lengh1 - 2 - i]);
        int c2 = tolower(str2[lengh2 - 2 - i]);
